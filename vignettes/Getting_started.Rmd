...
This function takes two arguments, the parameters we have previously generated `params`, and the `no_runs` which specifies the number of stochastic runs we want the model to run. This allows us to account for the randomness that is inherent in disease transmission. Increasing the number of runs also increases the length of time it takes, so you should choose a value that balances speed with certainty.

Here we will look at three runs of the model.
