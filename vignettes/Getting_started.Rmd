...
The first plot will depict the six compartments of the model, the total population, and the cases.

The second plot will show the susceptibility of the population at the final timepoint. This shows the proportion of each age group in one of five susceptibility labels.

1) **Susceptible** - Those who have not been exposed to vaccination or infection.
2) **Vaccine derived protection** - Those who have been exposed to vaccination but not infection.
3) **Infection derived protection** - Those who have been exposed to infection but not vaccination.
4) **Vaccine and infection derived protection** - Those who have been exposed to both infection and vaccination.
...
Next, we want to set up our onward scenario projections. In this example we will be looking forward 10 years, with vaccination steadily decreasing, cases being introduced in year 7, and R0 remaining constant. To model this forward projection we use the function `prepare_future_data()` which will take our original parameters generated with data_load_process_wrapper() `params`, the current population susceptibility, generated with `current_susceptibility()`, and a data.frame of future events. This future events data.frame should have three columns
...
#Model future cases
params_future <- prepare_future_data(
  params = params,
...
```{r, eval = F}
#Future results
run_future <- run_model_unpack_results(
  params_future,
...
#Generate plots
future_plots <- summary_plots(
  model_run = run_future,
